...
function exportNotesToFile() {
  ...
  link.download = `projectclock-export-${
  ...
  projects.forEach((project) => {
    ...
    card.innerHTML = `
      <div class="project-card-header">
        ...
        <button class="delete-project-btn" data-project-id="${
          project.id
      ...
      <div class="project-stats">
        <div class="project-stat">
          <span class="project-stat-value">${formatProjectTime(
            project.totalTime
          )}</span>
      ...
      <div class="project-created">Created ${formatDate(
        project.createdAt
      )}</div>
    `;
...
if (btnDeleteAllProjects) {
  btnDeleteAllProjects.addEventListener("click", () => {
    ...
    const confirmMessage = `Are you sure you want to delete ALL ${
      projects.length
    } projects?\n\nThis will permanently delete:\n• ${formatProjectTime(
      totalTime
...
document.addEventListener("click", (e) => {
  ...
  if (projectAimModal && !projectAimModal.hidden) {
    ...
    if (
      ...
      !showProjectAim.contains(e.target)
    ) {
      projectAimModal.hidden = true;
    }
  }
  ...
  if (raw && !hasProjects) {
    try {
      ...
      if (
        ...
        const migratedNotes = Array.isArray(s.notes)
          ? s.notes.map((note) => ({
              ...note,
